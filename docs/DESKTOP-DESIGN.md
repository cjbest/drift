...
The desktop package now lives in `drift-mac/`. Run npm, Cargo, and Playwright
commands from that directory. The `scripts/` helpers are at the repository root.
...
and wrapping scenarios. Visual fixtures are in [desktop-evidence](desktop-evidence/).
