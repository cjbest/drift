...
class BulletSpacer extends WidgetType {
  ...
  toDOM() {
    ...
    el.style.cssText = `display:inline-block;width:0;margin-left:-${this.offset}px`;
    return el;
  }
}
        ...
        for (
          ...
          if (list) {
            ...
            add(
              ...
              line.from,
              Decoration.line({
                attributes: { style: `margin-left:${offset}px` },
              }),
            );
            add(
              line.from,
              line.from,
              Decoration.widget({ widget: new BulletSpacer(offset), side: -1 }),
            );
          }
